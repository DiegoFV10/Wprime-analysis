...
void compare(string MCfile, string hist, string label, string name){
  ...
  /// W offshell ///
  TH1D* histA = (TH1D*) f1->Get("histograms/Wlnu");
  TH1D* histB = (TH1D*) f1->Get("histograms/Wonshell");
  ...
  /// W offshell ///
  // Compute MC ratio of W mass-binned vs W+2j inclusive
  TH1D* rateA = (TH1D*)histA->Clone("rateA");
  rateA->Divide(histB);
  ...
  frame->GetYaxis()->SetTitle("m_{W} binned / inclusive");
  ...
  leg->SetBorderSize(0);
  leg->SetTextSize(0.034);
  leg->AddEntry(histA, "madgraph m_{W}-binned", "apel");
  leg->AddEntry(histB, "amc@nlo inclusive", "apel");
  leg->Draw();

  //TString title = "#bf{CMS} #it{Simulation}                                    2022 postEE (13.6 TeV)"; /// W boosted
  TString title = "#bf{CMS} #it{Simulation}                                    2022 preEE (13.6 TeV)"; /// W offshell
