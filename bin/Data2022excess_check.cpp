...
void plotChecks(string filename, string selection){
  ...
  for(int i = 0; i < nentries; i++){
    ...
    double mT;
    if (selection == "presel")
      mT = sqrt( 2*mu_TunePRelPt[muIdx]*mu_pt[muIdx]*MET_pt*(1.0 - cos(mu_phi[muIdx] - MET_phi)) );
    else
    ...
    if (mu_pt[muIdx]*mu_TunePRelPt[muIdx] >= 500 && mu_pt[muIdx]*mu_TunePRelPt[muIdx] <= 700){// && goodJets_corr->size() == 0){// && mu_eta[muIdx] >= 0.5 && mu_eta[muIdx] <= 1.25){
      ...
      //cout << "Muon ==> pt: " << mu_pt[muIdx] << "  TuneP: " << mu_TunePRelPt[muIdx] << "  eta: " << mu_eta[muIdx] << "  phi: " << mu_phi[muIdx] << endl;
	...
	for (int j = 0; j < nJet; j++){
	  ...
	  muonEtaJetEta->Fill(mu_eta[muIdx], jet_eta[j]);
	...
	for (size_t j = 0; j < goodJets_corr->size(); j++){
	  ...
	  muonEtaJetEta->Fill(mu_eta[muIdx], jet_eta[goodJets_corr->at(j)]);

	} // Loop over jets
      }
      for (int e = 0; e < nElectron; e++){
	
	eleEta->Fill(el_eta[e]);
	elePhi->Fill(el_phi[e]);
	eleHEEP->Fill(el_HEEP[e]);
	muonEtaEleEta->Fill(mu_eta[muIdx], el_eta[e]);
