...
#error_poi = fpf_s.find(options.poi).getError() #KYL
...
for i in range(fpf_s.getSize()):
        ...
        if nuis_x == None:
            ...
            if nuis_p != None:
                ...
                if sigma_p > 0:
                    ...
                    print("No definition for prefit uncertainty %s. Printing absolute shifts"%(nuis_p.GetName())) #KYL
    ...
    #row += ["%+4.2f" % fit_s.correlation(name, options.poi)] #KYL
    #row += ["%+4.3f" % (nuis_x.getError() * fit_s.correlation(name, options.poi) * error_poi)] #KYL
...
for n in names:
    ...
    if (n, "b") in isFlagged:
        ...
        v[-2] = highlighters[isFlagged[(n, "b")]] % v[-2] #KYL
    if (n, "s") in isFlagged:
        #v[-2] = highlighters[isFlagged[(n, "s")]] % v[-2]
        v[-1] = highlighters[isFlagged[(n, "s")]] % v[-1] #KYL
    if options.format == "latex":
        n = n.replace(r"_", r"\_")
    if options.absolute_values:
        #print(fmtstring % (n, v[0], v[1], v[2], v[3], v[4]))
        print(fmtstring % (n, v[0], v[1], v[2], 0., 0.)) #KYL
    else:
        #print(fmtstring % (n, v[0], v[1], v[2], v[3]))
        print(fmtstring % (n, v[0], v[1], 0., 0.)) #KYL
...
if options.plotfile:
    ...
    canvas_nuis = ROOT.TCanvas("nuisances", "nuisances", 1200, 400) # Diego: antes (900, 600)
    ...
    hist_prefit.GetXaxis().SetLabelSize(0.025) # Diego
    hist_prefit.Draw("E2")
